...
#pragma omp critical
  ...
  if (read1[HEAD][0] != BEGIN || read1[PLUS][0] != PLUSCHAR || read2[HEAD][0] != BEGIN || read2[PLUS][0] != PLUSCHAR)
    exit(error("", ERRFASTQ));
               ...
               char **read2, int len1, int len2, int pos)
{
  ...
  while (i < len1 && j < len2)
  {
    if (read1[SEQ][i] == 'N' || read2[SEQ + EXTRA + 1][j] == 'N' || read1[SEQ][i] != read2[SEQ + EXTRA + 1][j])
              ...
              char **read2, int len1, int len2, int pos)
{
  ...
  while (i < len1 && j < len2)
  {
    fputc((read1[SEQ][i] == 'N' || read2[SEQ + EXTRA + 1][j] == 'N') ? ':' : (read1[SEQ][i] == read2[SEQ + EXTRA + 1][j] ? '|' : ' '), aln.f);
    i++;
    j++;
  }
  fputc('\n', aln.f);
              ...
              char **match, char **mism, omp_lock_t *lock)
{
  ...
  if (logOpt)
  {
    ...
    fprintf(log.f, "%s\t%d\t%d\t", header + 1,
            pos < 0 ? (len2 + pos < len1 ? len2 + pos : len1) : (len1 - pos < len2 ? len1 - pos : len2), len2 + pos);
...
void openWrite(char *outFile, File *out, bool gz)
{
  ...
  if (gz)
  {
    if (!strcmp(outFile + strlen(outFile) - strlen(GZEXT), GZEXT) || !strcmp(outFile, "/dev/null"))
    ...
    {
      // add ".gz" to outFile
      char *outFile2 = memalloc(strlen(outFile) + strlen(GZEXT) + 1);
  ...
  {
    out->f = (strcmp(outFile, "-") ? fopen(outFile, "w") : stdout);
    if (out->f == NULL)
      exit(error(outFile, ERROPENW));
  }
}
               ...
               bool adaptOpt, bool gz, bool interOpt)
{
  ...
  if (adaptOpt)
  {
    ...
    else
    {
      ...
      int add = strlen(ONEEXT) > strlen(TWOEXT) ? strlen(ONEEXT) + 1 : strlen(TWOEXT) + 1;
...
bool openRead(char *inFile, File *in)
{
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    if (j == EOF)
      exit(error(inFile, ERROPEN));
    if ((i && (unsigned char)j != 0x8B) || (!i && (unsigned char)j != 0x1F))
    {
      gzip = false;
      break;
    }
              ...
              char ***match, char ***mism)
{
  ...
  while (getLine(line, MAX_SIZE, qual, gz) != NULL)
  {
    ...
    else
    {
      ...
      i++;
      if ((arr == *match && ++matIdx > maxQual) || (arr == *mism && ++misIdx > maxQual))
        arr = NULL;
    }
  }
...
void run_rust(int maxQual,
              char *seq1, char *seq2, char *qual1,
            char *qual2, int len1, int len2, int overlap,
            bool dovetail, int doveOverlap, float mismatch,
            bool maxLen, float *best, int offset, bool fjoin){
  printf("abcde");
  printf("c offset: %d;", offset);
  char *qualFile = NULL;
  char **match = NULL, **mism = NULL; 
  saveQual(qualFile, maxQual,
              match, mism);
  int pos = findPos(seq1, seq2, qual1,
            qual2, len1, len2, overlap,
            dovetail, doveOverlap, mismatch,
            maxLen, best);
  printf("c pos: %d;", pos);
  if (pos != len1 - overlap + 1){
    createSeq(seq1, seq2, qual1,
               qual2, len1, len2, pos,
               offset, match, mism, fjoin);
  }
                ...
                bool verbose, int threads)
{
  ...
  while (file1 && file2)
  {
    ...
    // close input files
    if ((gz1 && gzclose(in1.gzf) != Z_OK) || (!gz1 && fclose(in1.f)) || (!inter && ((gz2 && gzclose(in2.gzf) != Z_OK) || (!gz2 && fclose(in2.f)))))
      exit(error("", ERRCLOSE));
...
void getArgs(int argc, char **argv)
{
  ...
  while ((c = getopt_long(argc, argv, OPTIONS, long_options, NULL)) != -1)
    ...
    case ALNFILE:
      ...
      break;
    case OVERLAP:
      overlap = getInt(optarg);
      break;
    case DOVEOVER:
      doveOverlap = getInt(optarg);
      break;
    case MISMATCH:
      mismatch = getFloat(optarg);
      break;
    case QUALITY:
      offset = getInt(optarg);
      break;
    case SETQUAL:
      maxQual = getInt(optarg);
      break;
    case QUALFILE:
      qualFile = optarg;
      break;
    case THREADS:
      threads = getInt(optarg);
      break;
    default:
      exit(-1);
    }
